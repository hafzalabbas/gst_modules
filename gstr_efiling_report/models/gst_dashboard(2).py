...
class gst_dashboard(models.Model):
    ...
    @api.one
    def _get_count(self):
        not_upload_count = len(self.env['account.invoice'].search(
            [('invoice_type', '=', self.invoice_type), ('gst_status', '=', 'not_uploaded')]))
        ready_count = len(self.env['account.invoice'].search(
            [('invoice_type', '=', self.invoice_type), ('gst_status', '=', 'ready_to_upload')]))
        uploaded_count = len(self.env['account.invoice'].search(
            [('invoice_type', '=', self.invoice_type), ('gst_status', '=', 'uploaded')]))
        filed_count = len(self.env['account.invoice'].search(
            [('invoice_type', '=', self.invoice_type), ('gst_status', '=', 'filed')]))
    ...
    def getTotalAmount(self, gst_status):
        inr_total = 0
        invoiceObjs = self.env['account.invoice'].search(
            [('invoice_type', '=', self.invoice_type), ('gst_status', '=', gst_status)])
