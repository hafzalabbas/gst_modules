...
class AccountPeriod(models.Model):
    ...
    date_start = fields.Date('Start of Period', default=fields.Date.context_today, required=True)
    date_stop = fields.Date('End of Period', default=fields.Date.context_today, required=True)

    @api.depends('date_stop')
    def _get_special(self):
        DATETIME_FORMAT = "%Y-%m-%d %H:%M:%S"
        currentDate = datetime.now() - timedelta(days=30)
        from_dte = currentDate.strftime(DATETIME_FORMAT)
        from_dt = datetime.strptime(from_dte, '%Y-%m-%d %H:%M:%S').date()
        for obj in self:
            if obj.date_stop < from_dt:
                obj.special = True
    ...
    #     for obj in self:
    #         if obj.date_stop!=False:
    #             date_stop = (obj.date_stop).strftime(DATETIME_FORMAT)
    #             if date_stop < from_dt:
    #                 obj.special = True
