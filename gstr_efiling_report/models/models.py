...
class GSTRSaleWizard(models.TransientModel):
    _name = 'invoice.report.gstsale.wizard'
    ...
    def generate_gstrsale_report(self):
        ...
        self.write({'state': 'get',
                    'report': out,
                    'name': 'gstr1_b2b' + str(from_date) + '-' + str(to_date) + '.xls'
                    })
        return {
            'type': 'ir.actions.act_window',
            'res_model': 'invoice.report.gstsale.wizard',
            'view_mode': 'form',
            'view_type': 'form',
            'res_id': self.id,
            'views': [(False, 'form')],
    ...
    def generate_gstr1_report(self, wb1):
        ...
        ws1.write_merge(row, row, col + 1, col + 6, "GSTR1 - B2B", header_content_style)
        ...
        ws1.write(row, col + 9, "Rate", sub_header_style)
        ws1.write(row, col + 10, "Taxable Value", sub_header_style)
        ...
        for invoice in self.sorted_invoices.filtered(
                lambda p: p.partner_id.x_gstin and p.partner_id.country_id.code == 'IN'):  # GST registered customers
        ...
        for invoice, inv_tax_lines in sorted(invoice_gst_tax_lines.items(), key=lambda p:(p[0].date, p[0].number)): # invoice_gst_tax_lines.items():
            for tax_id, base_amount in inv_tax_lines.items():
                ...
                ws1.write(row, col + 1, invoice.partner_id.x_gstin or "", line_content_style)
                ...
                ws1.write(row, col + 5, stateName, line_content_style)
                ws1.write(row, col + 6, self.reverse_charge and "Y" or "N", line_content_style)
                ...
                ws1.write(row, col + 9, tax_id, line_content_style)
                ws1.write(row, col + 10, base_amount[0], line_content_style)
                ws1.write(row, col + 11, base_amount[1], line_content_style)    #Cess amount
