...
class GSTRB2CSWizard(models.TransientModel):
    _name = 'invoice.report.gstb2cs.wizard'
    ...
    def generate_gstrsale_report(self):
        ...
        self.write({'state': 'get',
                    'report': out,
                    'name': 'gstr1_b2cs' + str(from_date) + '-' + str(to_date) + '.xls'
                    })
        return {
            'type': 'ir.actions.act_window',
            'res_model': 'invoice.report.gstb2cs.wizard',
            'view_mode': 'form',
            'view_type': 'form',
            'res_id': self.id,
            'views': [(False, 'form')],
    ...
    def generate_gstr1_report(self, wb1):
        ...
        ws1.write_merge(row, row, col + 1, col + 6, "GSTR1 - B2CS", header_content_style)
            ...
            for invoice_line in invoice.invoice_line_ids:
                ...
                if grouped_tax_lines.get(stateName):
                    if grouped_tax_lines[stateName].get(rate):
                        ...
                        grouped_tax_lines[stateName][rate][1] += gstDict['csamt']
                    else:
                        grouped_tax_lines[stateName][rate] = [0, 0]
                        grouped_tax_lines[stateName][rate][0] = line_taxes['total_excluded']
                        grouped_tax_lines[stateName][rate][1] = gstDict['csamt']
                else:
                    grouped_tax_lines[stateName] = {rate: [0,0]}
        ...
        for place_of_supply, inv_tax_lines in grouped_tax_lines.items(): # invoice_gst_tax_lines.items():
            for tax_id, base_amount in inv_tax_lines.items():
                ...
                ws1.write(row, col + 1, "OE", line_content_style)
                ...
                ws1.write(row, col + 5, base_amount[1], line_content_style)
