...
class GstInvoiceData(models.TransientModel):
    ...
    def getGSTInvoiceData(self, invoiceObj, invoiceType, data, gstType=''):
        ...
        for invoiceLineObj in invoiceObj.invoice_line_ids:
            if invoiceLineObj.product_id in pd_list:
                continue
            ...
            if invoiceLineData:
                ...
                cess = invoiceLineData[1].get('csamt') or 0.0
                ...
                if gstType == 'gstr2':
                    ...
                    if rate not in rateDict.keys():
                        rateDataDict[rate] = {'taxval':rateAmount, 'igst':igst, 'cgst':cgst, 'sgst':sgst, 'cess':cess}
                    else:
                        ...
                        rateDataDict[rate]['cess'] = rateDataDict[rate]['cess'] + cess
                # if gstType == 'gstr1':
                #     if rate not in rateDict.keys():
                #         rateDataDict[rate] = {'taxval':rateAmount, 'cess':cess}
                #     else:
                #         rateDataDict[rate]['taxval'] = rateDataDict[rate]['taxval'] + rateAmount
                #         rateDataDict[rate]['cess'] = rateDataDict[rate]['cess'] + cess
                ...
                invData = []
                # if gstType == 'gstr1':
                #     invData = invoiceLineData[0] + [rateDataDict[rate]['taxval']] + [rateDataDict[rate]['cess']]
                if gstType == 'gstr2':
                    if invoiceType in ['imps', 'impg']:
                        invData = invoiceLineData[0] + [rateDataDict[rate]['taxval'], rateDataDict[rate]['igst']] + [rateDataDict[rate]['cess']]
                    else:
                        invData = invoiceLineData[0] + [rateDataDict[rate]['taxval'], rateDataDict[rate]['igst'], rateDataDict[rate]['cgst'], rateDataDict[rate]['sgst']] + [rateDataDict[rate]['cess']]
                if invoiceType == 'b2b':
                    # if gstType == 'gstr1':
                    #     invData = invData
                    if gstType == 'gstr2':
                        invData = invData + ['Ineligible'] + [0.0] + [0.0] + [0.0] + [0.0]
                elif invoiceType == 'b2bur':
                    invData = invData  + ['Ineligible'] + [0.0] + [0.0] + [0.0] + [0.0]
                elif invoiceType in ['imps', 'impg']:
                    invData = invData  + ['Ineligible'] + [0.0] + [0.0]
    ...
    def getInvoiceLineData(self, invoiceLineData, invoiceLineObj, invoiceObj, invoiceType):
        ...
        _logger.info('Invoice (%s).',invoiceLineObj.invoice_id.name)
        if invoiceLineObj.discount_amount and invoiceLineObj.quantity != 0:
            price = invoiceLineObj.price_unit - (invoiceLineObj.discount_amount / invoiceLineObj.quantity)
        elif invoiceLineObj.discount:
        ...
        if rateObjs:
            ...
            jsonLineData = self.env['gst.tax.data'].getGstTaxData(invoiceObj, invoiceLineObj, rateObjs, taxedAmount, invoiceType,tax_details)
        else:
            ...
            lineData.append(0)
            tax_details = []
            jsonLineData = self.env['gst.tax.data'].getGstTaxData(invoiceObj, invoiceLineObj, False, taxedAmount, invoiceType,False)
        # rateAmount = invoiceLineObj.price_subtotal
        # rateAmount = rateAmount
        # if currency.name != 'INR':
        #     rateAmount = rateAmount * currency.rate
